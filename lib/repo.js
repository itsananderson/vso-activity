...
function branches(repositories, basePath, cb) {
    cb(undefined, repositories.map(function(repository) {
        return {
            ...
            branches: new repo(path.join(basePath, repository.project.name, repository.name))
                .BranchesSync()
                .map(function (b) {
                    return b.Name;
                })
...
function commits(repositories, basePath, cb) {
    cb(undefined, repositories.map(function(repository) {
        ...
        var commits = {};
        var branches = new repo(path.join(basePath, repository.project.name, repository.name))
                .BranchesSync()
                .filter(function(b) {
